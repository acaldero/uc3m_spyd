...
* El ciclo de trabajo típico es un bucle con los siguientes pasos en cada iteración:

  * <details>
    <summary>1. Editar el archivo fuente (o archivos) con la funcionalidad deseada. (click to expand)</summary>

	* Se parte de un archivo vacío...
      * En una primera pasada se añade un comentario de código por cada paso de lo que se quiere hacer.
      * En una siguiente pasada para cada comentario se añade el código fuente asociado después de dicho comentario.
      * En siguientes pasadas se refina el código fuente para mejorarlo según las directrices usadas.
	* Se parte de un ejemplo similar...
      * Se copia, pega y modifica el código de partida.
        * Los problemas suelen estar en acordarse de hacer todas las modificaciones correctamente.

    </details>
  * <details>
    <summary>2. Compilar el archivo fuente a binario. (click to expand)</summary>

	* Eliminar errores de compilación.

    </details>
  * <details>
    <summary>3. Ejecutar el archivo binario. (click to expand)</summary>
...
* A continuación se muestran estos pasos aplicados al ejemplo de "hola mundo".
...
#### 1.3 Ejecutar "hola mundo" en nodo local
...
#### 1.3.B Ejecutar "hola mundo" en varios nodos
...
* Para ejecutar en dos nodos hay que hacer tres pasos:
  ...
  2. Ha de tener el ejecutable en todos los nodos (si no se tiene un directorio de cuenta compartido en las máquinas):
    ...
    scp hola_mpi nodo1:~/lab_mpi/hola_mpi
    ssh nodo2 mkdir -p ~/lab_mpi
    scp hola_mpi nodo2:~/lab_mpi/hola_mpi
    ```
  3. Ha de lanzarse la ejecución en las máquinas deseadas usando ```mpirun```:
    ```bash
    cd ~/lab_mpi
    mpirun -np 4 -machinefile machines --map-by node ~/lab_mpi/hola_mpi
    ```
...
* Para ejecutar en dos nodos hay que hacer tres pasos:
  ...
  2. Ha de tener el ejecutable en todos los nodos (si no se tiene un directorio de cuenta compartido en las máquinas):
     ```bash
     cd ~/lab_mpi
     scp sr_mpi nodo1:~/lab_mpi/sr_mpi
     scp sr_mpi nodo2:~/lab_mpi/sr_mpi
     ```
  3. Ha de lanzarse la ejecución en las máquinas deseadas usando ```mpirun```:
     ```bash
     mpirun -np 2 -machinefile machines --map-by node ~/lab_mpi/sr_mpi
     ```
...
* Para ejecutar en dos nodos hay que hacer tres pasos:
  ...
  2. Ha de tener el ejecutable en todos los nodos (si no se tiene un directorio de cuenta compartido en las máquinas):
     ```bash
     scp pi_mpi nodo1:~/lab_mpi/pi_mpi
     scp pi_mpi nodo2:~/lab_mpi/pi_mpi
     ```
  3. Ha de lanzarse la ejecución en las máquinas deseadas usando mpirun:
     ```bash
     mpirun -np 2 -machinefile machines --map-by node ~/lab_mpi/pi_mpi
     ```
...
* Para ejecutar con MPI, se puede usar:
  ```bash
  time mpirun -np 8 ./pi_mpi_1000000000
  ```
