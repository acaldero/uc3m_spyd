...
DOCKER_PREFIX_NAME=spyd_docker
mkdir -p export
...
do
        ...
        case $arg_i in
             clean-build)
                ...
                spyd_docker_build $HOST_UID $HOST_GID "--no-cache"
             ;;
             ...
             build)
                ...
                spyd_docker_build $HOST_UID $HOST_GID ""
             ;;
             ...
             mpirun)
                ...
                # Check params
                if [ "x$CNAME" == "x" ]; then
                    echo ": There is not a running uc3m_spyd container."
                    exit
                fi
